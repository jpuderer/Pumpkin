 ...
 */
public class LedControlLocal implements AutoCloseable {
    ...
    public LedControlLocal(String spiGpio, int numDevices) throws IOException {
    ...
    public void reset(boolean clearDisplay) throws IOException {
        for (int i = 0; i < maxDevices; i++) {
            spiTransfer(i, OP_DISPLAYTEST, 0);
            setScanLimit(i, 7); // scanlimit: 8 LEDs
            spiTransfer(i, OP_DECODEMODE, 0); // decoding： BCD
            if (clearDisplay) clearDisplay(i);
            // we go into shutdown-mode on startup
            shutdown(i, true);
        }
    }
